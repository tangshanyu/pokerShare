...
export const ImportModal: React.FC<ImportModalProps> = ({ isOpen, onClose, onImport }) => {
  ...
  const handleImport = () => {
    ...
    lines.forEach(line => {
      // Expected format: Name Buyins Chips OR Name (if just adding names)
      ...
      if (parts.length > 0) {
        ...
        if (parts.length >= 2) {
          ...
          // Allow 0 as a valid buy-in count. Only default to 1 if it's not a number.
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in">
      <div className="glass-panel w-full max-w-lg rounded-2xl overflow-hidden shadow-2xl bg-[#1a1a20]/90 border border-white/10">
